...
import com.github.fgoncalves.analyser.di.DispatchersContainer
import com.github.fgoncalves.analyser.di.XmlParserFactory
 ...
 * Analyser service
 */
internal interface AnalyserParserService {
  /**
...
@Singleton
internal class DefaultAnalyserParserService(
  ...
  override fun analyse(uri: URI): Flow<List<Suite>> = flow {
    val handler = Handler()
    xmlParserFactory.xmlParser().parse(uri.toString(), handler)

    emit(handler.suites)
  }.flowOn(dispatchersContainer.io())
